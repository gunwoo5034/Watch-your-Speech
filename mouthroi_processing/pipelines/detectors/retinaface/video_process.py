...
class VideoProcess:
    ...
    def crop_patch(self, video, landmarks):
        ...
        for frame_idx, frame in enumerate(video):
            ...
            transformed_frame_face, transformed_landmarks_face = self.affine_transform(frame,smoothed_landmarks,self.reference,grayscale=False)
            ...
            patch_face = cut_patch(transformed_frame_face, transformed_landmarks_face, 80,80)
            sequence_face.append(patch_face)
            sequence.append(patch)
        return np.array(sequence) , np.array(sequence_face)
