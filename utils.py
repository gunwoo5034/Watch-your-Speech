...
def local_directory(name, model_cfg, diffusion_cfg, save_dir, output_directory):
    ...
    if save_dir is None:
        save_dir = os.getcwd() #현재 디렉토리 반환
    if not (name is None or name == ""):
        output_directory = os.path.join(save_dir, 'exp', name, local_path, output_directory) #name = LRS, output_directory = 'checkpoint'
    else:
...
def calc_diffusion_hyperparams(T, beta_0, beta_T, beta=None, fast=False):
    ...
    if fast and beta is not None:
        ...
        Beta = torch.linspace(beta_0, beta_T, T) #Beta 작은 간격으로 생성
    Alpha = 1 - Beta #alpha 정의
    Alpha_bar = Alpha + 0
    Beta_tilde = Beta + 0
    for t in range(1, T):
