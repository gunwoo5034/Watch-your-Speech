...
def sampling(net, diffusion_hyperparams, w_video, condition=None):
    ...
    mouthroi, face_image,text = condition
    sample_step = 30
    ...
    with torch.no_grad():
        for i in range(sample_step):
            ...
            v_pred_con    = net.sample(x, eps0, mouthroi, face_image, text, diffusion_steps, cond_drop_prob=0)   # predict \epsilon according to \epsilon_\theta
            v_pred_uncond = net.sample(x, eps0, mouthroi, face_image, text, diffusion_steps, cond_drop_prob=1)
            v_pred = v_pred_uncond + w_video * (v_pred_con - v_pred_uncond)

            x = x - v_pred * dt
            # epsilon_theta = net(x, mouthroi, face_image,text, diffusion_steps, cond_drop_prob=0)   # predict \epsilon according to \epsilon_\theta
            # epsilon_theta_uncond = net(x, mouthroi, face_image,text, diffusion_steps, cond_drop_prob=1)
...
def generate(
    ...
    for i in dataset_indices:
        _gt_melspec, _, _mouthroi, _face_image, _text, _ = dataset[i]
    ...
    for i in tqdm(range(n_samples)):
        _melspec = sampling(
            ...
            w_video,
            condition=(mouthroi[i].cuda(), face_image[i].cuda(), text[i]),
        )
